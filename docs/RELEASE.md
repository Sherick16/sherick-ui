...
A `main` push passes all three CI verification jobs before the publish job checks npm.
It skips a version already live and rejects a downgrade; a newer stable version is
submitted with `npm stage publish` and provenance, **not published immediately**.
A maintainer reviews the staged tarball and approves it with 2FA on npmjs.com or via
`npm stage approve <stage-id>`; only then does it become available under `latest`.
Pending staged versions are not visible to `npm view … version`, so avoid approving
an older pending stage after a newer version is live.

The release owner must configure npmjs.com's trusted publisher for owner `Sherick16`,
repository `sherick-ui`, workflow filename **`ci.yml`** (not the full path), no
environment, and the **stage-only allowed action**. Node 24 and npm 11.15.0+ are
required for staged publishing; CI installs the supported npm CLI explicitly.
No npm token is stored in GitHub; only the publish job has `id-token: write`. Neither
PRs nor failed verification stage a package. Do not manually publish, create release
tags, change dist-tags or bypass verification without explicit release-owner approval.
...
The three independent CI jobs verify the exact `main` commit. Once they pass, the
publish job installs with the frozen lockfile and runs
`npm stage publish --access public --provenance` from `packages/ui`. Its `prepack` builds
artifacts and stages the bundled patched Base UI dependency; `postpack` restores the
workspace link. A maintainer must review and approve the staged version with 2FA
before it reaches npm `latest`. Main pushes queue rather than cancel an in-progress
stage. There is no release tag generated by CI; manual checks below remain a
separate human responsibility when relevant.
